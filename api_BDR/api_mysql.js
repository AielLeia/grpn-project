...
app.post('/login/Conexion', function(req, res) {
...
app.get('/infosDeCompte/:pseudo', function (req, res) {
  ...
  console.log(req.params);
    connection.query("SELECT * FROM  COMPTE where pseudo = '"+req.params.pseudo+"'", function (error, results, fields) {
...
app.post('/creationDeCompte', function(req, res) {
  ...
  console.log(req.body);
  connection.query('insert into COMPTE (pseudo,motdepasse) values (?,?);',[req.body.pseudo,req.body.motdepasse], function (err, result) {
        if (err) throw err;
...
app.put('/modificationDeCompte',(req, res) => {
    var sql = "UPDATE COMPTE SET pseudo='"+req.body.pseudo+"', motdepasse='"+req.body.motdepasse+"' WHERE pseudo="+req.params.pseudo;
...
const port = 3000;
app.listen(port, ()=> {
  console.log('le serveur fonctionne pour l"API mariadb fonctionne sur le port : '+ port );
});